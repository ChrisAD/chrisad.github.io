...
I found a command injection on a website hosted in an Azure web app. That much is the customer's problem, not Microsoft's, and I agreed with that from the start. The interesting part was what I could do next. Azure ships Kudu with every web app, reachable by adding `scm` to the hostname. Kudu's Process Explorer lets an administrator open a process and view its full environment. Filenames cannot contain `<` or `>`, but environment variables can hold anything, and my command injection let me set them. So I set an environment variable to `<script>alert(1)</script>` and waited for the SaaS administrator to open Process Explorer, where my JavaScript would run in their authenticated Kudu session. From there you swap `alert(1)` for a BeEF hook and go to work on the operator, not just the customer.

The catch was persistence. A normal process runs, does its work, and exits, taking my poisoned environment with it. Azure would simply spawn a fresh worker and the payload would be gone before any administrator ever looked. To make the XSS stick I needed the process to stay alive and visible in Process Explorer, so I forced a denial of service on the worker: after setting the environment variable I ran a command that hung indefinitely (`tasklist` did this reliably on Azure at the time). The process never exited, the poisoned environment never got cleaned up, and the payload sat there waiting. The DoS was not the goal, it was the delivery mechanism. It turned a transient injection into a persistent, admin-facing stored XSS.
